...
class WordsComparison:
    ...
    def predict(self, query_object: List[str], top_k: int = 10) -> List[int]:
...
class TfidfSimilarity:
    ...
    def fit(self, text_data: pd.DataFrame) -> None:
        ...
        self.word_vectorizer = TfidfVectorizer(analyzer='word', ngram_range=(1, 2), max_df=0.8, smooth_idf=True,
                                               norm='l2', stop_words=list(stopwords.words('english')))
    ...
    def predict(self, query_object: List[str], top_k: int = 10) -> ndarray[Any, dtype[signedinteger[Any] | long]]:
...
class ObjectsSimilarityFiltered:
    ...
    def predict(self, query_object_text: List[str], filter_features: List[int], top_k: int = 10) -> ndarray[Any, dtype[
        signedinteger[Any] | long]] | list[Any]:
